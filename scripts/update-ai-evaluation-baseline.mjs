...
function assertPassingCoverage(result) {
  if (!result?.passed || result.cases?.some(item => item.passed !== true || item.hardFailures?.length) || result.tools?.some(item => item.passed !== true)) throw new Error("evaluation is not fully passing");
  const tools = [...new Set(result.tools.map(item => item.tool))].sort();
  const ages = [...new Set(result.cases.map(item => item.ageBand))].sort();
  if (JSON.stringify(tools) !== JSON.stringify(REQUIRED_TOOLS) || JSON.stringify(ages) !== JSON.stringify(REQUIRED_AGES)) throw new Error("evaluation coverage is incomplete");
}
...
export async function runCli(args = process.argv.slice(2), options = {}) {
  ...
  try {
    const result = await refreshEvaluationBaseline({ repoRoot: path.resolve(options.repoRoot ?? path.join(import.meta.dirname, "..")), evaluate: options.evaluate, io: options.io, testHooks: options.testHooks, datasets: options.datasets });
    stdout(`baseline refresh: passed (${result.bytesChanged ? "updated" : "unchanged"})\n`);
    return 0;
