...
The CLI does not write by default. `--output <filename>` explicitly selects a directly contained filename in the canonical repository root; nested destinations, lexical escapes, symlinks, hard links, and junctions are rejected. Root `dev`/`ino` identity is rechecked before temporary-file open, after temporary open and write, immediately before rename, and after rename, with temporary/destination identity and link-count checks around installation. An already-authorized malicious local actor racing after the final check is outside the no-secret report threat model. Add `--json` to write the stable JSON report instead of text.
