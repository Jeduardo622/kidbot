...
Normal evaluation reads `evals/baselines/ai-output-baseline.json` and blocks every negative delta, even above the absolute thresholds. `pnpm run eval:ai:update-baseline` is the explicit baseline refresh command; CI never runs it. The canonical baseline includes no timestamp, provider, network, or model judge data, so case-set drift stays visible and must be reviewed before refresh.

GitHub Actions uses its native `GITHUB_ACTIONS=true` and absolute `GITHUB_STEP_SUMMARY` values to append an ephemeral changed-only job summary from the existing single evaluation. The section includes status, the full fingerprint, absolute totals, changed case/tool/overall metrics, drift reasons, and the unchanged count; unchanged metrics are not listed individually. It performs no persistent repository write, PR comment, provider or network request, artifact upload, payload/path disclosure, or secret handling. Local and incomplete-environment runs make no summary filesystem operation. The canonical Markdown is bounded to 32 KiB. An active summary error emits only `evaluation: summary error` and exits `3`; evaluation failure `1` and invalid baseline `2` keep precedence until a valid comparison exists.

The CLI does not write by default. `--output <path>` explicitly selects a direct child file in either the canonical repository root or the operating-system temporary directory; nested destinations, lexical escapes, symlinks, hard links, and junctions are rejected. The selected root's `dev`/`ino` identity is rechecked before temporary-file open, after temporary open and write, immediately before rename, and after rename, with temporary/destination identity and link-count checks around installation. An already-authorized malicious local actor racing after the final check is outside the no-secret report threat model. Add `--json` to write the stable JSON report instead of text.
