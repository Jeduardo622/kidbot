...
5. Run the existing manual protected post-deploy workflows for widget distribution, provider-backed story panels, parent Redis create/history/delete and provider roundtrip. They may incur provider costs and synthetic writes; dispatch only against the reviewed target and approval.
6. Record actual generated output/retrieval and parent-store cleanup, then verify all four activities in the supported host. Health checks alone do not prove credentials are accepted by OpenAI or that generated content is suitable.
