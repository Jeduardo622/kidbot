...
Normal evaluation compares against `evals/baselines/ai-output-baseline.json`; every negative delta blocks even when absolute thresholds pass. `pnpm run eval:ai:update-baseline` is the explicit refresh command and remains absent from CI. The canonical baseline contains no timestamp, provider, network, or model judge data, and case-set drift must be reviewed before refresh.

In GitHub Actions, the existing single evaluator run appends an ephemeral changed-only job summary when `GITHUB_ACTIONS` is exactly `true` and `GITHUB_STEP_SUMMARY` is a nonempty absolute path. It shows status, fingerprint, absolute totals, changed metrics, drift reasons, and the unchanged count, but no persistent repository write, PR comment, provider, network request, payload, path, or secret. Normal local and incomplete-environment runs do not touch a summary file. Summary Markdown is bounded to 32 KiB; an active summary error exits `3` with a generic message, while evaluation failure `1` and invalid baseline `2` retain precedence before a valid comparison exists.
